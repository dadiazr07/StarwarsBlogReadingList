                ...
                <div className="d-grid gap-2 d-md-flex justify-content-md-end position-absolute bottom-0 end-0 mb-3 me-3">
                    ...
                    <a onClick={(e) => { 
                        if (store.allData.length < 1) {
                            actions.addAllData()
                            actions.addToFav(starship.url)
                        } else { actions.addToFav(starship.url) }
                        }} className="btn btn-dark btn-sm text-warning">{ !store.favorites.some((el) => el.url == starship.url) ? <>♡</> : <>♥</>}</a>
                </div>
            </div>
        </div>
    );
